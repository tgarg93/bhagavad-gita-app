...
- **The only real consumer of any onboarding answer is Krishna's context block** (`krishnaContextService.buildContextBlock()`), which pastes it into his Gemini prompt — that part is genuine, on every chat, reflection and follow-up. **Nothing else reads the profile**: not the journey, not Home, not the Learn tab, not notifications (a "5 min/day" user and a "20 min/day" user get byte-identical reminders). So **no question may imply a payoff outside Krishna without wiring one first.** Waiting to be wired, if a future question wants to earn its keep: `STAGE_2_DEITIES` (`journeyPath.ts`) is a flat id list a deity answer could reorder, and the notification hours are hardcoded next to an existing `NotificationSettings` object.
...
- **The gate is curriculum tuning, not enforcement.** The Foundations track sums to **87 points** (32 cards + 20 graded checks × 2 + 1 reflection) — deliberately short of the 100 for Shishya, so a reader who does every card and check but skips the capstone sits visibly short of it. Passing the capstone confers the level *and* adds 30 (→117), so it then holds on points alone. **Adding a reflection (×15), or enough checks to cross 100, levels the reader up mid-track and deflates the capstone.** Checks are worth 2, so ~26 fit under the gate; if the content changes, redo the sum (commented at the top of `foundations.ts`).
...
- `progressToNext` is clamped at **both** ends. A rite conferring level 2 at 87 points makes `points - level.minPoints` negative, which renders a backwards progress bar without the lower clamp.
