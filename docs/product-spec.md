...
- **Path screen** (`JourneyPath` route) + onboarding finale share `JourneyPathView`: intro card ("How this path is laid" — the five-stage rationale), module milestone cards with pure-View progress rings, expandable checklists with 40px cover tiles (done = faded + teal tick; next = saffron frame + NEXT pill; Gita tiles get chapter-number overlays). Current module auto-expands. Nothing is locked.
