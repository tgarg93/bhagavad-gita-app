...
- Every clip reads the card's **takeaway headline first, then its body** (including
  any on-screen bullets, read as a spoken list) — matching what's on screen so the
  sentence-by-sentence highlight lines up. (Sanskrit terms are respelled for the voice
  — see the key below — so the audio text and the on-screen text differ by a few
  letters per term; the highlight maps by sentence, so this is fine.)
...
The voice reads **English spelling**, so it mangles Sanskrit baked into the prose
(it says "jnana" as *juh-NAH-nuh*, "atman" as *AT-man*). The blocks below are already
respelled to steer it. **If a term still sounds wrong, change it here and in the
blocks** — that's faster than changing voices. Common, dictionary-adopted words
(karma, dharma, moksha, yoga, Krishna, Rama, Shiva, Vishnu, Ganesha, Diwali, Gita,
Mahabharata) are left alone because the voice already handles them.

| Written on screen | Respelled for the voice |
|---|---|
| atman | **aatman** |
| Brahman | **Brahmun** *(not "Brahmin")* |
| Sanatana Dharma | **Sanaatana Dharma** |
| prana | **praana** |
| gunas | **goonas** |
| jnana | **gyaana** |
| raja (yoga) | **Raaja** |
| shruti | **shrooti** |
| purusharthas | **purushaarthas** |
| ishta-devata | **ishta-dayvata** |
| Trimurti | **Trimoorti** |
| Shaktas / Smartas | **Shaaktas / Smaartas** |
| murti / puja / prasad | **moorti / pooja / prasaad** |
| kama | **kaama** |
| Kali / Sita / Ravana | **Kaali / Seeta / Raavana** |
| Sindhu | **Sindhoo** |
| Navaratri / Janmashtami | **Nava-raatri / Janmaashtami** |

**Pacing & pauses.** ElevenLabs runs sentences together, which made the read feel
rushed. Fix it with two levers:
...
- **Punctuation & short sentences** — periods and em-dashes already give small pauses;
  the rewrite is short-sentenced on purpose.
- Optional global lever: lower `speed` in the generator from `1.0` to ~`0.92`.

Convention below: `<break time="0.9s" />` after the takeaway, `0.6s` between paragraphs,
`0.4s` between items in a spoken list.

**The runnable source is `scripts/generate-foundations-audio.mjs`** — extend its
`SCRIPTS` object from these blocks (break tags and all) and re-run to regenerate.
**File names never change** — they're keyed to the permanent section ids.
...
**`f-claim-brahman.mp3`**
> Behind every god and every form is a single reality. It is called Brahmun. `<break time="0.9s" />` So what is behind all those faces? The four streams are all reaching for the same thing. `<break time="0.6s" />` They call it Brahmun — not a god sitting somewhere watching you, but the ground of everything that is: pure awareness, without edges and without a face. `<break time="0.6s" />` Every deity you're about to meet is a face placed on Brahmun, so that a human being has something to love and hold onto. The formless is hard to pray to; a face is not.

**`f-claim-atman.mp3`**
> The same one reality is also what looks out from inside you — your true self, aatman. `<break time="0.9s" />` Now turn the telescope around, from the whole universe to you. `<break time="0.6s" />` Notice that you can watch your own thoughts arrive and pass. Whatever is doing the watching — the awareness behind your eyes — is aatman, your true self. `<break time="0.6s" />` It isn't the body, which changes with age. It isn't your mood, which comes and goes. Aatman is the witness that has been there the whole time, unchanged since you were a child.

**`f-claim-tat-tvam-asi.mp3`**
> Brahmun and aatman are not two things — they are one. That is the whole claim. `<break time="0.9s" />` Here is the turn the whole tradition is built on: Brahmun, the one reality, and aatman, your deepest self, are not two things. They are one and the same. `<break time="0.6s" />` Picture a clay pot sitting out in the open air. The space inside the pot and the vast space of the sky look separate, but they aren't. The pot has walls; the space doesn't. Break the pot and nothing is released, because there was only ever one space, briefly shaped. `<break time="0.6s" />` You are the space inside the pot. You are not standing near the divine — you are made of it, and have simply forgotten.
...
**`f-claim-prana.mp3`**
> One living current runs through all of it — breath, body, wind. It is called praana. `<break time="0.9s" />` Behind the misreading, one living current runs through everything. It is called praana. `<break time="0.6s" />` Praana means breath — but not only breath. It is the animating energy in a body, a tree, a gust of wind: the tradition's bridge between the physical and the spiritual, the life that moves. `<break time="0.6s" />` It is also the most practical idea in this whole part. It is why nearly every Hindu practice — yoga, meditation, chanting — begins at the breath. Steady the breath, and you have a handle on the mind.

**`f-claim-gunas.mp3`**
> Everything nature makes is woven from three strands — the three goonas. `<break time="0.9s" />` Everything that current flows through — every person, every mood, every moment — is woven from three strands, called the goonas. `<break time="0.6s" />` They aren't three types of person, and they aren't good, better, and best. All three run in everyone at once; only the ratio shifts through the day. The work isn't to erase any of them — it is to notice which one is running you right now. `<break time="0.6s" />` Sattva — clarity, lightness, balance. `<break time="0.4s" />` Rajas — heat, drive, restlessness. `<break time="0.4s" />` Tamas — inertia, heaviness, fog.
...
**`f-wheel-aims.mp3`**
> You are not required to sprint for the exit — pleasure and prosperity are proper goals too. `<break time="0.9s" />` You might expect the goal to be renouncing everything and racing for moksha. It isn't. `<break time="0.6s" />` Hinduism names four proper aims of a human life — the purushaarthas. Look at what made the list: this is not an ascetic religion that grudgingly tolerates the world. It is a world-affirming one that simply keeps an exit door open at the back. `<break time="0.6s" />` Dharma — to live rightly. `<break time="0.4s" />` Artha — to prosper, to build and provide for others. `<break time="0.4s" />` Kaama — to enjoy: desire, pleasure, love, beauty. `<break time="0.4s" />` Moksha — to be free of the whole cycle.

**`f-wheel-yogas.mp3`**
> There are four roads to that exit, each matched to the kind of person you already are. `<break time="0.9s" />` And there isn't just one road to that exit. There are four — called the yogas — and each suits a different kind of person. `<break time="0.6s" />` None outranks the others. You don't have to become someone else to arrive; you simply take the road that fits how you are already built. `<break time="0.6s" />` Bhakti — the path of love and devotion, if your heart leads. `<break time="0.4s" />` Karma yoga — the path of selfless work, if you would rather act than sit. `<break time="0.4s" />` Gyaana — the path of knowledge, if you must reason it through. `<break time="0.4s" />` Raaja — the path of meditation, if you can sit still.
...
**`f-faces-shakti.mp3`**
> Not one of the gods can act without her. The Goddess is power itself — Shakti. `<break time="0.9s" />` So far the gods have been male. Here is the twist: not one of them can act without her. `<break time="0.6s" />` Shakti means energy, capability, power — the very ability to do anything at all. Shiva without his Shakti is famously pictured as inert, a corpse. She is the force behind every god's action. `<break time="0.6s" />` And she is one Goddess with many tempers: Parvati is Durga is Kaali — the gentle wife, the lion-riding warrior, and the terrifying one who ends things. For millions of Hindus, She is the supreme reality, full stop.
...
every takeaway the reader banked — the same thirty-two lines, in order. Record it as a
single file. The closing question that follows it is the interactive capstone (the
reader answers it aloud in their own words); it is not a recorded card.

**`f-capstone-recap.mp3`**
> You can say all of this now. Thirty-two sentences, and you arrived with none of them. `<break time="0.9s" />`
>
> Hinduism has no founder, no single book, and no one in charge. Even the name isn't its own — it's a river, mispronounced. The name it gives itself is Sanaatana Dharma — the eternal way. An eternal way still has to be carried — so Sanskrit was built to be remembered, not read. `<break time="0.6s" />`
>
...
>
> Behind every god and every form is a single reality: Brahmun. The same one reality is also what looks out from inside you — aatman. Brahmun and aatman are not two things; they are one. The world only seems separate because it is misread, not unreal — that misreading is maya. One living current runs through all of it: praana. And everything nature makes is woven from three strands — the goonas. `<break time="0.6s" />`
>
