...
- `telemetryService` — Sentry (crashes) + PostHog (anonymous product events); env-gated, silently no-ops without keys. **Events carry ids/counts/levels only — never reflection text, chat content, or profile fields.** Capture points: `journeyService.markCompleted`, `foundationsService.recordCapstone` + `recordCheck` (first pass only), `geminiService.sendMessage`, `JourneyCelebration` level-up, onboarding complete in App.tsx, `content_reader_opened` (ContentReaderScreen), `daily_chai_viewed` `{atomType, sourceRef?}` (HomeScreen — `sourceRef` is a content id, never profile text), `$screen` route changes (AppNavigator `onStateChange` → `trackScreen`), `ask_krishna_opened` `{source: fab|tab}` (reader FABs + AskKrishnaScreen focus), `krishna_chat_limit_reached` (chatQuotaService, once per day at exhaustion). `@sentry/react-native` is a **native dep** (post-add builds are native, not OTA).
